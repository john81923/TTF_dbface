...
class BBox:
    ...
    def safe_scale_center_and_diff(self, scale, limit_x, limit_y):
        ...
        return [int(cx+0.5), int(cy+0.5), cx - int(cx+0.5), cy - int(cy+0.5)]
...
#(239, 121, 162)
def drawbbox(image, bbox, Namefile, color=None, thickness=1, textcolor=(0, 0, 0), landmarkcolor=(0, 0, 255)):
    ...
    cv2.putText(image, Namefile[cls_id], (x,y), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 2)
    ...
    if bbox.haslandmark:
        for i in range(len(bbox.landmark)):
            ...
            cv2.circle(image, intv(x, y), 2, landmarkcolor, -1, 16)
...
def parse_facials_webface_center(facials):

    ts = []
    for facial in facials:
        label = int(facial[0])
        x, y, w, h = facial[1:5]
        box = [x-w/2, y-h/2, x + w/2 - 1, y + h/2 - 1]
        landmarks = None

        if w * h < 4 * 4:
            continue

        ts.append(BBox(label=label, xyrb=box, landmark=landmarks, rotate = False))
    return ts
...
if __name__ == '__main__':
    org_label = "/mnt/sdd/craig/face_detection/webface/train/label.txt"
    label_file = '/mnt/sdd/johnnysun/Dataset/platemania_webcrawler/us_trainval/DBocr/label.txt'
    files = load_webface( label_file, "/mnt/sdd/craig/face_detection/webface/WIDER_train/images")
    print(files[0] )
    #['/mnt/sdd/craig/face_detection/webface/WIDER_train/images/0--Parade/0_Parade_marchingband_1_849.jpg', [(BBox[facial]: x=449.00, y=330.00, r=570.00, b=478.00, width=122.00, height=149.00, landmark=[488.906, 373.643],[542.089, 376.442],[515.031, 412.83],[485.174, 425.893],[538.357, 431.491])]]
