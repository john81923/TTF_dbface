...
def augmentWithCropScaleWebface(image, objs, outw=800, outh=800, mode='normal', keepsize=8, max_rotate_angle=20):
    ...
    for i in range(len(objs) - 1, -1, -1):
        ...
        if ioumin < 0.9 or objs[i].area < keepsize * keepsize:
            del objs[i]

    if len(objs) < 4 : #and randrf(0, 1) > 0.8:
...
def webface(image, objs, outw=800, outh=800, keepsize=8):
    ...
    #funcs = [[augmentWithColorJittering, 0.7], [augmentationMotionBlur, 0.6], \
    #            [augmentationAdditiveGaussianNoise, 0.6], [augmentationBlendAlphaVerticalLinearGradient,0.5] ]
    funcs = [[augmentWithColorJittering, 0.7], \
                [augmentationAdditiveGaussianNoise, 0.5] ]
    ...
    #    image, objs = augmentWithCropScaleWebface(image, objs, outw, outh, 'cube', keepsize=keepsize, max_rotate_angle=0)
    #else:
    image, objs = augmentWithCropScaleWebface(image, objs, outw, outh, keepsize=keepsize, max_rotate_angle=12)
