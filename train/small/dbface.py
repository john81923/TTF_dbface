...
class mobilenetv2_100(nn.Module):
    def __init__(self):
        ...
        backbone = effnet._gen_mobilenet_v2('mobilenetv2_100', 1.0, pretrained=True)
...
class Mbv3SmallFast(nn.Module):
    def __init__(self):
        ...
        self.bneck = nn.Sequential(
            ...
            Block(3, 16, 72, 24, nn.ReLU(inplace=True), None, 1),               # 1
...
class DBFace(nn.Module):
    def __init__(self, has_landmark=False, wide=24, has_ext=False, upmode="UCBA", classes=35):
        ...
        self.up2 = UpModule(wide, wide, kernel_size=2, stride=1, mode=upmode) # s4
        self.detect = DetectModule(wide)

        self.center = HeadModule(wide, classes, has_ext=has_ext)
    ...
    #torch.Size([18, 3, 80, 240])
    #torch.Size([18, 64, 3, 8])  s32
    #torch.Size([18, 48, 5, 15]) s16
    #torch.Size([18, 64, 6, 16]) up s32
    #torch.Size([18, 64, 5, 15]) connnect s16

    #torch.Size([18, 64, 25, 25])
    #torch.Size([18, 48, 50, 50])
    #torch.Size([18, 64, 50, 50])
    #torch.Size([18, 64, 50, 50])
    ...
    def forward(self, x):
        ...
        #print('s32\t',s32.shape)
        s32 = self.conv3(s32)
        #print('conv3 s32\t',s32.shape)

        #print('up0 s32\t',self.up0(s32).shape)
        #print('connect2 s16\t',self.connect2(s16).shape)
        #print()

        s16 = self.up0(s32) + self.connect2(s16)
        s8 = self.up1(s16) + self.connect1(s8)
        s4 = s8 + self.connect0(s4)    # took of the up2 module
        x = self.detect(s4)
...
class DBFace_mbv2(nn.Module):
    def __init__(self, has_landmark=False, wide=24, has_ext=False, upmode="UCBA", classes=None):
        super(DBFace_mbv2, self).__init__()
        self.has_landmark = has_landmark

        # define backbone
        self.bb = mobilenetv2_100()
...
def generate_onnx(model, save_path):
    model.eval()
    model.cuda()

    img = torch.zeros((1, 3,64, 160), device='cuda')
    #model = Model('/home/doris/yolov5-v3_1/models/yolov5s.yaml', nc=80)
    #model.load_state_dict (torch.load('/mnt/models/Object_models/YOLOv5/yolov5s_v3_state_dict.pt') )
    torch.onnx.export( model, img, f'{save_path}.onnx', verbose=False, opset_version=9,  keep_initializers_as_inputs=True,
                            input_names=['images'], output_names=['hm','tlbr'])

if __name__=='__main__':
    save_path = '/mnt/models/CarLicensePlates_Detection_Recognition/ocr/testing/ocr_mbv3_initw_Hsigmoid.pth'
    #kmodel = DBFace(has_landmark=False, wide=64, has_ext=True, upmode="UCBA", classes=35)
    #kmodel.init_weights()
    #kmodel.eval()
    model = DBFace(has_landmark=False, wide=64, has_ext=True, upmode="UCBA", classes=35)
    model.eval()
    model.load(save_path)

    generate_onnx( model, save_path[:-4] )
