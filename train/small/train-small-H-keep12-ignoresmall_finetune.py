...
class VDataset(Dataset):
    def __init__(self, imagesdir, mean, std):
        self.items = glob('%s/IMG*.jpg'%imagesdir)
        self.mean = mean
        self.std = std
    ...
    def __getitem__(self, index):
        ...
        image = common.imread(imgfile)
        image = ((image / 255.0 - self.mean) / self.std).astype(np.float32)
        return T.to_tensor(image), basename(splitext(imgfile)[0]).split('_')[2]



class App(object):
    def __init__(self, labelfile, imagesdir, valimgdir, load_checkpoint=None):

        self.width, self.height = 256, 96
        ...
        if load_checkpoint and isfile(load_checkpoint):
    ...
    def val(self, epoch):
        log.info(f'Evalutation {len(self.val_dataset)}')
        tp = 0
        with torch.no_grad():
            ocr_count = ocr_top1 = ocr_frg = ocr_fn = ocr_fp = 0
            for imgval, valgt_lp in tqdm(self.val_loader):
                ...
                for lp_top2, gt_lp in zip(val_result, valgt_lp):
                    ...
                    if lp_top2:
                        ...
                        ocr_fn += 1
            log.info(f" epoch: {epoch} - top1_val:{ocr_top1/len(self.val_dataset):.2f} fp_val:{ocr_fp/len(self.val_dataset):.2f} frg_val:{ocr_frg/len(self.val_dataset):.2f}  ocr_count:{ocr_count/len(self.val_dataset):.2f} fn_val:{ocr_fn/len(self.val_dataset):.2f} ")
...
trial_name = "small-UCBA-mbv2-tw"
jobdir = f"jobs"

log = logger.create(trial_name, f"{jobdir}/logs/trial.log")
...
app = App('/mnt/sdd/johnnysun/Dataset/car_plates/AOLP/Crop_LP/label.txt', \
            '/mnt/sdd/johnnysun/Dataset/car_plates/AOLP/Crop_LP/images', \
            '/mnt/sdd/johnnysun/Dataset/car_plates/AOLP/Crop_LP/images' , \
            '/mnt/sdd/johnnysun/Car_License_DetReg/TTF_OCR-dbface/train/small/jobs/small-UCBA-mbv2-center/models/151.pth' )
app.train()
